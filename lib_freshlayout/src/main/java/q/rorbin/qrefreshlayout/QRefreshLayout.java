...
public class QRefreshLayout extends FrameLayout implements NestedScrollingChild, NestedScrollingParent {
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        if (mRefreshEnable && dy > 0 && (mMovedDisY > 0 || !mHeaderView.canTargetScroll()) && mMode == MODE_REFRESH) {
            if (dy > mMovedDisY) {
    ...
    public void onNestedScroll(View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        if (mRefreshEnable && dy < 0 && !canTargetScrollUp()) {
            mMode = MODE_REFRESH;
            handleHeaderScroll(-dy);
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                mTouchY = currY;
                if (dy > 0 && mRefreshEnable && !canTargetScrollUp() && mFooterView.canTargetScroll()) {
                    mMode = MODE_REFRESH;
