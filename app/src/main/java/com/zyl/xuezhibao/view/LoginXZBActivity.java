...
import com.zyl.xuezhibao.view_.TextLoginActivity;
...
public class LoginXZBActivity extends MyBaseActivity implements View.OnClickListener {

    /**
     * 注册
     */
    private Button mActivityLoginRegister;
    ...
    private void gotoActivity(Class<?> class1) {
        Intent intent = new Intent(LoginXZBActivity.this, class1);
    ...
    public void initView() {
        ...
        mActivityLoginRegister = (Button) findViewById(R.id.btn_login_register);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_login_register://注册
                gotoActivity(PhoneRegisterActivity.class);
                break;
    ...
    private void loginFinish(String phone,String password) {
        ...
        OkHttpUtils.post().url(Constants.Url.Login_URL).params(requestMap).build().execute(new StringCallback() {
            ...
            public void onError(Call call, Exception e, int id) {
                ...
                ToastUtils.showShort(LoginXZBActivity.this,"网络异常！");
            }
            @Override
            public void onResponse(String response, int id) {
                ...
                try {
                    ...
                    ToastUtils.showShort(LoginXZBActivity.this,object.getString("message"));
                    if ("success".equals(object.getString("status"))){
                        mSharePerence.setMemberInfo(resultData);
//                        UMengAliasUtils.setUMengAlias(LoginXZBActivity.this,mSharePerence.getMemberInfo().getPhone());
    ...
    private void getUserMesg(final String access_token, final String openid) {
        String path = "https://api.weixin.qq.com/sns/userinfo?access_token="
                ...
                + "&openid="
                + openid;
        OkHttpUtils.post().url(path).build().execute(new StringCallback() {
            @Override
            public void onError(Call call, Exception e, int id) {
            }
            ...
            @Override
            public void onResponse(String response, int id) {
                Globals.log("log LoginActivity:  getUserMesg_result:" + response);
                String deviceToken = BaseSharePerence.getInstance(getApplicationContext()).getDeviceToken();
//                WeChatInfo info = JSON.parseObject(response, WeChatInfo.class);
//                BindPhone bindPhone = new BindPhone();
//                bindPhone.setUnionid(info.getUnionid());
//                bindPhone.setNickname(info.getNickname());
//                bindPhone.setOpenId(info.getOpenid());
//                bindPhone.setImage(info.getHeadimgurl());
//                bindPhone.setDeviceToken(deviceToken);
//                weChatLogin(bindPhone);
            }
        });
    }
